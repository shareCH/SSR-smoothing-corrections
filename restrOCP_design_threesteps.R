...
restrOCP_design_threesteps <- function(design,t1,delta){
  ...
  third <- (tlow - qnorm(1-design$alpha_0))/3 + qnorm(1-design$alpha_0)
  twothird <- 2*(tlow - qnorm(1-design$alpha_0))/3 + qnorm(1-design$alpha_0)
  n_left_1 <- design$n_1
  n_left_2 <- (design$f*design$n_1 - design$n_1)/3 + design$n_1
  n_left_3 <- 2*(design$f*design$n_1 - design$n_1)/3 + design$n_1
  n <- ifelse(t1>= qnorm(1-design$alpha_1) | t1< qnorm(1-design$alpha_0), design$n_1, 
              ifelse(n_recalc < design$f*design$n_1, n_recalc, ifelse(0.6 <= cp & cp < 0.8, design$f*design$n_1, 
                                                                      ifelse(t1 >= qnorm(1-design$alpha_0)& t1 <= third, n_left_1, 
                                                                             ifelse(t1 > third & t1 <= twothird, n_left_2, n_left_3)))))
